...
public sealed class FamilyThumbnailPreviewService
{
	...
	private static string BuildOrderedStringStamp(IEnumerable<string> values)
	{
		return string.Join("|", (from x in values ?? Enumerable.Empty<string>()
			...
			where x.Length > 0
			select x).OrderBy([SpecialName] (string x) => x, StringComparer.Ordinal));
	}
	...
	private static string BuildParameterCacheStamp(IEnumerable<StandardFamilyParameterSnapshotItem> parameters)
	{
		return string.Join("|", (from x in parameters ?? Enumerable.Empty<StandardFamilyParameterSnapshotItem>()
			where x != null
			select Normalize(x.Scope) + ":" + Normalize(x.TypeName) + ":" + Normalize(x.Name) + ":" + Normalize(x.StorageType) + ":" + Normalize(x.ValuePreview) + ":" + x.IsShared + ":" + Normalize(x.ExternalGuid) + ":" + Normalize(x.ParameterId)).OrderBy([SpecialName] (string x) => x, StringComparer.Ordinal));
	}
	...
	private static string BuildNestedFamilyCacheStamp(IEnumerable<StandardNestedLoadableFamilySnapshotItem> items)
	{
		return string.Join("|", (from x in items ?? Enumerable.Empty<StandardNestedLoadableFamilySnapshotItem>()
			where x != null
			select Normalize(x.CategoryName) + ":" + Normalize(x.FamilyName) + ":" + x.TypeCount.ToString(CultureInfo.InvariantCulture) + ":" + BuildOrderedStringStamp(x.TypeNames) + ":" + x.IsShared).OrderBy([SpecialName] (string x) => x, StringComparer.Ordinal));
	}
	...
	private static string SafeFileName(string value, int maxLength = 120)
	{
		...
		string safe = new string(value.Select([SpecialName] (char ch) => (!invalidFileNameChars.Contains(ch)) ? ch : '_').ToArray()).Trim().TrimEnd('.', ' ');
