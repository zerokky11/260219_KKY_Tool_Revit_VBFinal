...
public sealed class ProjectComparisonReviewExcelExportService
{
	...
	private static List<List<string>> BuildRows(ProjectStandardComparisonReport report, string disciplineLabel, string comparisonReportPath, bool korean)
	{
		...
		if (report.LoadableFamilies != null)
		{
			foreach (LoadableFamilyComparisonItem item in report.LoadableFamilies.Where([SpecialName] (LoadableFamilyComparisonItem x) => x != null && IsReviewExportStatus(x.Status)).OrderBy([SpecialName] (LoadableFamilyComparisonItem x) => x.CategoryName ?? string.Empty, StringComparer.OrdinalIgnoreCase).ThenBy([SpecialName] (LoadableFamilyComparisonItem x) => x.FamilyName ?? string.Empty, StringComparer.OrdinalIgnoreCase))
			{
		...
		if (report.ProjectLoadableSignatureFailures != null)
		{
			foreach (ProjectLoadableSignatureFailureItem item3 in report.ProjectLoadableSignatureFailures.Where([SpecialName] (ProjectLoadableSignatureFailureItem x) => x != null).OrderBy([SpecialName] (ProjectLoadableSignatureFailureItem x) => x.CategoryName ?? string.Empty, StringComparer.OrdinalIgnoreCase).ThenBy([SpecialName] (ProjectLoadableSignatureFailureItem x) => x.FamilyName ?? string.Empty, StringComparer.OrdinalIgnoreCase))
			{
				rows.Add(new List<string>
				{
					discipline,
	...
	private static string ExtractSignatureDebugIds(string value)
	{
		...
		foreach (Match match in matches)
		{
			if (match != null && match.Groups.Count >= 2)
			{
				...
				if (!string.IsNullOrWhiteSpace(idValue) && !ids.Contains(idValue, StringComparer.OrdinalIgnoreCase))
				{
					ids.Add(idValue);
				}
			}
