...
public sealed class RoutingDependencyPreflightService
{
	...
	public static RoutingDependencyPreflightPlan BuildPlan(SystemTypeCatalogSnapshot sourceCatalog, RoutingFamilyCatalogSnapshot targetCatalog)
	{
		...
		Dictionary<string, RoutingFamilyCatalogEntry> byLibraryId = source.Where([SpecialName] (RoutingFamilyCatalogEntry x) => !string.IsNullOrWhiteSpace(x.LibraryFamilyId)).GroupBy([SpecialName] (RoutingFamilyCatalogEntry x) => Normalize(x.LibraryFamilyId), StringComparer.Ordinal).ToDictionary([SpecialName] (IGrouping<string, RoutingFamilyCatalogEntry> x) => x.Key, [SpecialName] (IGrouping<string, RoutingFamilyCatalogEntry> x) => x.First(), StringComparer.Ordinal);
		Dictionary<string, List<RoutingFamilyCatalogEntry>> byFamilyName = source.GroupBy([SpecialName] (RoutingFamilyCatalogEntry x) => Normalize(x.FamilyName), StringComparer.Ordinal).ToDictionary([SpecialName] (IGrouping<string, RoutingFamilyCatalogEntry> x) => x.Key, [SpecialName] (IGrouping<string, RoutingFamilyCatalogEntry> x) => x.ToList(), StringComparer.Ordinal);
		_Closure_0024__3_002D0 closure_0024__3_002D = default(_Closure_0024__3_002D0);
		foreach (SystemTypeSemanticSnapshot sourceType in sourceCatalog.Types.OrderBy([SpecialName] (SystemTypeSemanticSnapshot x) => Normalize(x.SystemFamilyKind), StringComparer.Ordinal).ThenBy([SpecialName] (SystemTypeSemanticSnapshot x) => Normalize(x.TypeName), StringComparer.Ordinal))
		{
			using IEnumerator<RoutingDependencySnapshot> enumerator2 = sourceType.RoutingDependencies.OrderBy([SpecialName] (RoutingDependencySnapshot x) => Normalize(x.DependencyRole), StringComparer.Ordinal).ThenBy([SpecialName] (RoutingDependencySnapshot x) => Normalize(x.FamilyName), StringComparer.Ordinal).ThenBy([SpecialName] (RoutingDependencySnapshot x) => Normalize(x.TypeName), StringComparer.Ordinal)
				.GetEnumerator();
			while (enumerator2.MoveNext())
			{
				...
				List<string> duplicateTypeNames = (from x in targetFamily.Types.Where(closure_0024__3_002D._Lambda_0024__13).Where(closure_0024__3_002D._Lambda_0024__14)
					select x.TypeName).Distinct(StringComparer.OrdinalIgnoreCase).OrderBy([SpecialName] (string x) => x, StringComparer.OrdinalIgnoreCase).ToList();
	...
	{
		return entries.SelectMany([SpecialName] (RoutingFamilyCatalogEntry x) => x.Types.Select([SpecialName] (RoutingFamilyTypeSnapshot t) => t.TypeName)).Distinct(StringComparer.OrdinalIgnoreCase).OrderBy([SpecialName] (string x) => x, StringComparer.OrdinalIgnoreCase);
	}
