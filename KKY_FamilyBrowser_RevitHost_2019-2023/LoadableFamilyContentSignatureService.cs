...
public sealed class LoadableFamilyContentSignatureService
{
	...
	private static string BuildElementSignature(Document familyDocument, bool includeAnnotationGraphics, IList<string> elementDebugLines = null)
	{
		...
		List<Element> elements = (from Element x in (IEnumerable)new FilteredElementCollector(familyDocument).WhereElementIsNotElementType()
			...
			select x).OrderBy([SpecialName] (Element x) => Normalize(((object)x).GetType().Name), StringComparer.Ordinal).ThenBy([SpecialName] (Element x) => Normalize(ResolveCategoryName(x)), StringComparer.Ordinal).ThenBy([SpecialName] (Element x) => Normalize(SafeElementName(x)), StringComparer.Ordinal)
			.ToList();
	...
	private static string BuildNestedFamilyLabelSignature(Document familyDocument, FamilyManager manager)
	{
		...
		try
		{
			List<FamilyInstance> instances = ((IEnumerable)new FilteredElementCollector(familyDocument).OfClass(typeof(FamilyInstance))).Cast<FamilyInstance>().OrderBy([SpecialName] (FamilyInstance x) => Normalize(ResolveCategoryName((Element)(object)x)), StringComparer.Ordinal).ThenBy([SpecialName] (FamilyInstance x) => Normalize(SafeElementName((Element)(object)x)), StringComparer.Ordinal)
				.ToList();
