...
public sealed class FamilyBrowserStandardListService
{
	...
	private static StandardListBaselineState BuildInitialBaselineExclusions(IEnumerable<FamilyBrowserStandardListEntry> entries, StandardLibrarySnapshot standardSnapshot, string disciplineKey, string disciplineLabel, string currentUser)
	{
		...
		foreach (StandardLoadableFamilySnapshotItem item in (standardSnapshot.LoadableFamilies ?? new List<StandardLoadableFamilySnapshotItem>()).Where([SpecialName] (StandardLoadableFamilySnapshotItem x) => x != null).OrderBy([SpecialName] (StandardLoadableFamilySnapshotItem x) => FoldToken(x.CategoryName), StringComparer.Ordinal).ThenBy([SpecialName] (StandardLoadableFamilySnapshotItem x) => FoldToken(x.FamilyName), StringComparer.Ordinal))
		{
		...
		foreach (StandardSystemTypeSnapshotItem item2 in (standardSnapshot.SystemTypes ?? new List<StandardSystemTypeSnapshotItem>()).Where([SpecialName] (StandardSystemTypeSnapshotItem x) => x != null).OrderBy([SpecialName] (StandardSystemTypeSnapshotItem x) => FoldToken(x.CategoryName), StringComparer.Ordinal).ThenBy([SpecialName] (StandardSystemTypeSnapshotItem x) => FoldToken(x.TypeClassName), StringComparer.Ordinal)
			.ThenBy([SpecialName] (StandardSystemTypeSnapshotItem x) => FoldToken(x.TypeName), StringComparer.Ordinal))
		{
	...
	private static string TryDiscoverListPath(FamilyBrowserStandardLibrarySlot slot, StandardLibraryRegistrationRecord registration)
	{
		...
		try
		{
			foreach (string candidate2 in (from filePath in Directory.EnumerateFiles(folder)
				...
				select filePath).OrderBy([SpecialName] (string filePath) => Path.GetFileName(filePath), StringComparer.OrdinalIgnoreCase))
			{
	...
	private static List<FamilyBrowserStandardListIndexItem> BuildStandardListIndexItems(FamilyBrowserStandardPolicy policy)
	{
		...
		return (from item in items
			...
			select @group.First()).OrderBy([SpecialName] (FamilyBrowserStandardListIndexItem item) => FamilyBrowserPolicyKey.Normalize(item.Discipline), StringComparer.Ordinal).ThenBy([SpecialName] (FamilyBrowserStandardListIndexItem item) => FamilyBrowserPolicyKey.Normalize(item.DisplayName), StringComparer.Ordinal).ToList();
	}
