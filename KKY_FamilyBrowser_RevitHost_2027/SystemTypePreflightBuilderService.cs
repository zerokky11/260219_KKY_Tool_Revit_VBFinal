...
public sealed class SystemTypePreflightBuilderService
{
	...
	public static int EnsureRoutingFamilyDependencyItems(SystemTypePreflightReport report, Document standardDocument, Document projectDocument)
	{
				...
				case "consolidateduplicatesuffixtypes":
				{
					...
					foreach (RoutingPreferenceRuleGroupType group in Enum.GetValues(typeof(RoutingPreferenceRuleGroupType)).Cast<RoutingPreferenceRuleGroupType>())
					{
						...
						catch (Exception ex)
						{
							...
							Exception ex2 = ex;
							AddRoutingDependencyDiagnostic(report.Diagnostics, item, "CannotVerifyRouting", "Could not read routing rule count. group=" + group.ToString() + " / " + ex2.Message);
							ProjectData.ClearProjectError();
							continue;
						}
						int num = ruleCount - 1;
						for (int index = 0; index <= num; index++)
						{
							...
							catch (Exception ex3)
							{
								...
								Exception ex4 = ex3;
								AddRoutingDependencyDiagnostic(report.Diagnostics, item, "CannotVerifyRouting", "Could not read routing rule. group=" + group.ToString() + " index=" + index.ToString(CultureInfo.InvariantCulture) + " / " + ex4.Message);
								ProjectData.ClearProjectError();
								continue;
							}
							if (RoutingRuleHasMappablePart(rule) && standardDocument.GetElement(rule.MEPPartId) is FamilySymbol { Family: not null } sourceSymbol)
							{
								string sourceFamilyName = (sourceSymbol.Family.Name ?? string.Empty).Trim();
								string sourceTypeName = ResolveElementName(sourceSymbol);
								string sourceFamilyKey = RoutingFamilyCatalogBuilder.BuildFamilyKey(ResolveCategoryName(sourceSymbol), sourceFamilyName);
								...
								if (!existingKeys.Contains(dependencyKey))
								{
									...
									report.Diagnostics.Add(new SystemTypePreflightDiagnostic
									{
										...
										Details = "group=" + group.ToString() + " index=" + index.ToString(CultureInfo.InvariantCulture) + " family=" + sourceFamilyName + " type=" + sourceTypeName
									});
								}
							}
						}
	...
	private static bool TargetSystemTypeMatchesStandardRoutingForPreflight(Document targetDocument, Document standardDocument, ElementType sourceType, ElementType targetType, ref string diagnosticReason, ref string diagnosticDetails)
	{
						...
						foreach (RoutingPreferenceRuleGroupType group in Enum.GetValues(typeof(RoutingPreferenceRuleGroupType)).Cast<RoutingPreferenceRuleGroupType>())
						{
							...
							if (sourceRules.Count == targetRules.Count)
							{
								...
								while (index <= num)
								{
									...
									{
										diagnosticReason = "CannotVerifyRouting";
										diagnosticDetails = "A routing rule could not be read. group=" + group.ToString() + " index=" + index.ToString(CultureInfo.InvariantCulture) + " sourceIndex=" + sourceRules[index].SourceIndex.ToString(CultureInfo.InvariantCulture) + " targetIndex=" + targetRules[index].SourceIndex.ToString(CultureInfo.InvariantCulture);
										TargetSystemTypeMatchesStandardRoutingForPreflight = false;
									}
									else
									{
										...
										if ((object)sourceRule.MEPPartId != null && sourceRule.MEPPartId != ElementId.InvalidElementId && sourcePart == null)
										{
											diagnosticReason = "CannotVerifyRouting";
											diagnosticDetails = "A standard routing rule part could not be resolved. group=" + group.ToString() + " index=" + index.ToString(CultureInfo.InvariantCulture) + " sourceIndex=" + sourceRules[index].SourceIndex.ToString(CultureInfo.InvariantCulture) + " sourcePartId=" + FormatElementId(sourceRule.MEPPartId);
											TargetSystemTypeMatchesStandardRoutingForPreflight = false;
										}
										else
										{
											...
											{
												diagnosticReason = "CannotVerifyRouting";
												diagnosticDetails = "A routing rule mapped part could not be resolved in the target. group=" + group.ToString() + " index=" + index.ToString(CultureInfo.InvariantCulture) + " sourceIndex=" + sourceRules[index].SourceIndex.ToString(CultureInfo.InvariantCulture) + " sourcePart=" + sourcePart.GetType().Name + ":" + ResolveElementName(sourcePart);
												TargetSystemTypeMatchesStandardRoutingForPreflight = false;
											}
											...
											else
											{
												...
												{
													diagnosticReason = "RoutingPreferenceMismatch";
													diagnosticDetails = "A routing part definition differs. group=" + group.ToString() + " index=" + index.ToString(CultureInfo.InvariantCulture) + " sourceIndex=" + sourceRules[index].SourceIndex.ToString(CultureInfo.InvariantCulture) + " part=" + sourcePart.GetType().Name + ":" + ResolveElementName(sourcePart);
													TargetSystemTypeMatchesStandardRoutingForPreflight = false;
												}
												else
												{
													...
													diagnosticReason = "RoutingPreferenceMismatch";
													diagnosticDetails = "Routing rule description or criteria differ. group=" + group.ToString() + " index=" + index.ToString(CultureInfo.InvariantCulture) + " sourceIndex=" + sourceRules[index].SourceIndex.ToString(CultureInfo.InvariantCulture) + " targetIndex=" + targetRules[index].SourceIndex.ToString(CultureInfo.InvariantCulture);
													TargetSystemTypeMatchesStandardRoutingForPreflight = false;
												}
											}
										}
							...
							diagnosticReason = "RoutingPreferenceMismatch";
							diagnosticDetails = "Effective routing rule count differs. group=" + group.ToString() + " source=" + sourceRules.Count.ToString(CultureInfo.InvariantCulture) + " target=" + targetRules.Count.ToString(CultureInfo.InvariantCulture);
							TargetSystemTypeMatchesStandardRoutingForPreflight = false;
							goto end_IL_004a;
						}
	...
	private static IEnumerable<string> CollectDependencyFamilyNames(SystemTypeCatalogSnapshot catalog)
	{
		return (from x in (catalog?.Types ?? new List<SystemTypeSemanticSnapshot>()).SelectMany([SpecialName] (SystemTypeSemanticSnapshot x) => x?.RoutingDependencies ?? new List<RoutingDependencySnapshot>())
			...
			select x.FamilyName).Distinct<string>(StringComparer.OrdinalIgnoreCase).ToList();
	}
	...
	private static void AppendApplyDryRunBlocks(SystemTypePreflightReport report, Document standardDocument, Document projectDocument)
	{
		...
		using (TransactionGroup group = new TransactionGroup(projectDocument, "KKY Family Browser System Type Apply Dry Run"))
		{
			try
			{
				group.Start();
	...
	private static void AppendNonFamilyRoutingPartBlocks(SystemSyncExecutionPlan executionPlan, Document standardDocument, Document projectDocument)
	{
		...
		checked
		{
			foreach (SystemSyncExecutionItem executionItem in executionPlan.Items ?? new List<SystemSyncExecutionItem>())
			{
				...
				foreach (NonFamilyRoutingPartCheck check in (from x in checks.Where([SpecialName] (NonFamilyRoutingPartCheck x) => x != null).GroupBy<NonFamilyRoutingPartCheck, string>([SpecialName] (NonFamilyRoutingPartCheck x) => x.Action + "|" + x.Status + "|" + x.Notes, StringComparer.OrdinalIgnoreCase)
					select x.First() into x
					orderby x.IsBlocking
					select x).ThenBy<NonFamilyRoutingPartCheck, string>([SpecialName] (NonFamilyRoutingPartCheck x) => x.Notes, StringComparer.OrdinalIgnoreCase))
				{
				...
				foreach (string issue in issues.Distinct<string>(StringComparer.OrdinalIgnoreCase).OrderBy<string, string>([SpecialName] (string x) => x, StringComparer.OrdinalIgnoreCase))
				{
					executionItem.Steps.Add(new SystemSyncExecutionStep
					{
						SequenceNo = executionItem.Steps.Count + 1,
	...
	private static NonFamilyRoutingPartCheck BuildNonFamilyRoutingPartCheck(Document standardDocument, Document projectDocument, Element sourcePart, RoutingPreferenceRuleGroupType group, int ruleIndex)
	{
		if (sourcePart is PipeSegment sourceSegment)
		{
		...
		if (targetPart == null)
		{
			return NonFamilyRoutingPartCheck.Block("ReviewRoutingPart", "ManualReview", "A non-family routing preference part is missing and does not yet have an explicit safe ensure path. Rule group: " + group.ToString() + " / Rule index: " + ruleIndex.ToString(CultureInfo.InvariantCulture) + " / Part: " + sourcePart.GetType().Name + " : " + ResolveElementName(sourcePart));
		}
		if (!RoutingPartDefinitionsMatch(standardDocument, sourcePart, projectDocument, targetPart))
		{
			return NonFamilyRoutingPartCheck.Block("ReviewRoutingPart", "ManualReview", "Routing part differs from the standard RVT. Rule group: " + group.ToString() + " / Rule index: " + ruleIndex.ToString(CultureInfo.InvariantCulture) + " / Part: " + sourcePart.GetType().Name + " : " + ResolveElementName(sourcePart));
		}
		return NonFamilyRoutingPartCheck.Ready("ReuseRoutingPart", "Ready", "Routing part can be reused. Rule group: " + group.ToString() + " / Rule index: " + ruleIndex.ToString(CultureInfo.InvariantCulture) + " / Part: " + sourcePart.GetType().Name + " : " + ResolveElementName(sourcePart));
	}

	private static NonFamilyRoutingPartCheck BuildPipeSegmentRoutingPartCheck(Document standardDocument, Document projectDocument, PipeSegment sourceSegment, RoutingPreferenceRuleGroupType group, int ruleIndex)
	{
		if (string.IsNullOrWhiteSpace(BuildPipeSegmentSizeSignature(sourceSegment)))
		{
			return NonFamilyRoutingPartCheck.Block("ReviewPipeSegment", "ManualReview", "The standard pipe segment size table could not be read. Rule group: " + group.ToString() + " / Rule index: " + ruleIndex.ToString(CultureInfo.InvariantCulture) + " / Segment: " + ResolveElementName(sourceSegment));
		}
		Element targetByName = ResolveMatchingTargetElement(projectDocument, sourceSegment);
		if (targetByName != null && RoutingPartDefinitionsMatch(standardDocument, sourceSegment, projectDocument, targetByName))
		{
			return NonFamilyRoutingPartCheck.Ready("ReusePipeSegment", "Ready", "Pipe segment can be reused. Rule group: " + group.ToString() + " / Rule index: " + ruleIndex.ToString(CultureInfo.InvariantCulture) + " / Segment: " + ResolveElementName(sourceSegment));
		}
		ReferenceMappingCheck materialCheck = ResolvePotentialMappedReference(projectDocument, standardDocument, sourceSegment.MaterialId, "pipe segment material");
		if (materialCheck.SourceReference == null)
		{
			return NonFamilyRoutingPartCheck.Block("ReviewPipeSegmentMaterial", "ManualReview", (materialCheck.Blocker ?? "The standard pipe segment material could not be resolved.") + " Segment: " + ResolveElementName(sourceSegment));
		}
		...
		{
			return NonFamilyRoutingPartCheck.Block("ReviewPipeSegmentSchedule", "ManualReview", (scheduleCheck.Blocker ?? "The standard pipe segment schedule type could not be resolved.") + " Segment: " + ResolveElementName(sourceSegment));
		}
		if (materialCheck.TargetReference != null && scheduleCheck.TargetReference != null)
		{
			...
			{
				if (RoutingPartDefinitionsMatch(standardDocument, sourceSegment, projectDocument, existingByCombination))
				{
					return NonFamilyRoutingPartCheck.Ready("ReusePipeSegment", "Ready", "Pipe segment can reuse an existing project segment with matching material, schedule type, and size table. Standard segment: " + ResolveElementName(sourceSegment) + " / Project segment: " + ResolveElementName(existingByCombination));
				}
		...
		{
			return NonFamilyRoutingPartCheck.Ready("ResolveStalePipeSegmentName", "Ready", "A same-name pipe segment exists with a non-standard material or schedule definition. Admin-authoritative apply will route to the correct material/schedule segment and report the stale segment for cleanup. Rule group: " + group.ToString() + " / Rule index: " + ruleIndex.ToString(CultureInfo.InvariantCulture) + " / Segment: " + ResolveElementName(sourceSegment));
		}
		return NonFamilyRoutingPartCheck.Ready("CreatePipeSegment", "Ready", "Pipe segment is missing and is planned for creation during the system type transaction. Rule group: " + group.ToString() + " / Rule index: " + ruleIndex.ToString(CultureInfo.InvariantCulture) + " / Segment: " + ResolveElementName(sourceSegment));
	}
	...
	private static List<string> FindNonFamilyReferenceIssues(Document standardDocument, Document projectDocument, ElementType sourceType)
	{
		...
		foreach (Parameter parameter in sourceType.Parameters.Cast<Parameter>())
		{
			...
			if (sourceReference != null && !(sourceReference is FamilySymbol) && !(sourceReference is Material) && !(sourceReference is PipeScheduleType) && !(sourceReference is PipeSegment) && !IsAuthoritativeRoutingDependencyReference(sourceReference))
			{
				...
				{
					issues.Add("Referenced system definition differs from the standard RVT. Parameter: " + ResolveParameterName(parameter) + " / Reference: " + sourceReference.GetType().Name + " : " + ResolveElementName(sourceReference));
				}
			}
		}
		return issues.Distinct<string>(StringComparer.OrdinalIgnoreCase).OrderBy<string, string>([SpecialName] (string x) => x, StringComparer.OrdinalIgnoreCase).ToList();
	}
	...
	private static ElementType FindSystemType(Document doc, string systemFamilyKind, string categoryName, string typeName)
	{
		...
		return new FilteredElementCollector(doc).WhereElementIsElementType().Cast<ElementType>().FirstOrDefault([SpecialName] (ElementType x) => x != null && (!(x is FamilySymbol) || SystemTypeDetailedComponentSnapshotService.SupportsRequiredCurtainPanelComponents(x.GetType().Name)) && string.Equals(x.GetType().Name, systemFamilyKind, StringComparison.OrdinalIgnoreCase) && CategoryNamesMatch(ResolveCategoryName(x), categoryName) && string.Equals(Normalize(ResolveElementName(x)), Normalize(typeName), StringComparison.Ordinal));
	}
	...
	private static bool RoutingPartDefinitionsMatch(Document sourceDocument, Element sourcePart, Document targetDocument, Element targetPart)
	{
		...
		if (!string.Equals(BuildPipeSegmentSizeSignature(sourceSegment), BuildPipeSegmentSizeSignature(targetSegment), StringComparison.Ordinal))
		{
			return false;
		}
		ReferenceMappingCheck materialCheck = ResolveExistingMappedReference(targetDocument, sourceDocument, sourceSegment.MaterialId, "pipe segment material");
		...
		return ElementIdsEqual(materialCheck.TargetReference.Id, targetSegment.MaterialId) && ElementIdsEqual(scheduleCheck.TargetReference.Id, targetSegment.ScheduleTypeId);
	}
	...
	private static string BuildPipeSegmentSizeSignature(Segment segment)
	{
		...
		else
		{
			...
			BuildPipeSegmentSizeSignature = string.Join("|", parts.OrderBy<string, string>([SpecialName] (string x) => x, StringComparer.Ordinal));
		}
		goto IL_014b;
		IL_014b:
