...
public class App : IExternalApplication
{
	...
	public Result OnStartup(UIControlledApplication application)
	{
		...
		try
		{
			...
			AddPushButton(panel, "KKYFamilyBrowserOpenDashboard", "Family\nBrowser", assemblyPath, "KKY_FamilyBrowser_RevitHost_2027.CmdOpenFamilyBrowser");
	...
	private void AddPushButton(RibbonPanel panel, string buttonName, string buttonText, string assemblyPath, string className)
	{
		...
		if (panel.GetItems().OfType<RibbonItem>().FirstOrDefault([SpecialName] (RibbonItem x) => string.Equals(x.Name, ((ItemData)val).Name, StringComparison.Ordinal)) == null)
		{
