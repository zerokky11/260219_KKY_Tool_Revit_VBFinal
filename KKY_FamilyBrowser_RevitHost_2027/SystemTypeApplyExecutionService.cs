...
public sealed class SystemTypeApplyExecutionService
{
	...
	public static SystemTypeApplyExecutionReport Execute(Document targetDocument, Document standardDocument, SystemTypePreflightReport preflightReport, string preflightPath, Action<int, int, string> progress = null)
	{
		...
		List<SystemSyncExecutionItem> executionItems = (preflightReport.ExecutionPlan?.Items ?? new List<SystemSyncExecutionItem>()).OrderBy<SystemSyncExecutionItem, string>([SpecialName] (SystemSyncExecutionItem x) => SystemTypeIdentityService.BuildKey(x.SystemFamilyKind, x.CategoryName, x.SystemTypeName), StringComparer.Ordinal).ToList();
	...
	private static void ApplyDependencyFamilies(Document targetDocument, Document standardDocument, IDictionary<string, Family> standardFamilyMap, IEnumerable<RoutingDependencyPreflightItem> dependencyItems, ISet<string> refreshedDependencyFamilies, SystemTypeApplyExecutionReport report, SystemTypeApplyExecutionItem resultItem)
	{
		...
		foreach (RoutingDependencyPreflightItem dependencyItem in dependencyItemList.OrderBy<RoutingDependencyPreflightItem, string>([SpecialName] (RoutingDependencyPreflightItem x) => Normalize(x.SourceFamilyName), StringComparer.Ordinal).ThenBy<RoutingDependencyPreflightItem, string>([SpecialName] (RoutingDependencyPreflightItem x) => Normalize(x.SourceTypeName), StringComparer.Ordinal))
		{
	...
	private static List<string> FindMissingOrDifferentStandardSizes(IEnumerable<MEPSize> sourceSizes, Segment targetSegment)
	{
		...
		return result.Distinct<string>(StringComparer.Ordinal).OrderBy<string, string>([SpecialName] (string x) => x, StringComparer.Ordinal).ToList();
	}
	...
	private static string BuildCopiedElementSummary(IEnumerable<Element> copiedElements)
	{
		List<string> values = (from x in copiedElements ?? new List<Element>()
			where x != null
			select ((object)x).GetType().Name + " : " + ResolveElementName(x)).Distinct<string>(StringComparer.OrdinalIgnoreCase).OrderBy<string, string>([SpecialName] (string x) => x, StringComparer.OrdinalIgnoreCase).ToList();
		if (values.Count == 0)
		{
			return "(none)";
		}
	...
	private static void GuardAgainstCopiedFamilies(Document targetDocument, ISet<int> familyStateBefore, IEnumerable<RoutingDependencyPreflightItem> dependencyItems = null)
	{
		...
		if (newFamilies.Count != 0)
		{
			List<string> unexpected = (from x in newFamilies.Where([SpecialName] (Family x) =>
				...
				select ((Element)x).Name ?? string.Empty).Distinct<string>(StringComparer.OrdinalIgnoreCase).OrderBy<string, string>([SpecialName] (string x) => x, StringComparer.OrdinalIgnoreCase).ToList();
	...
	private static IEnumerable<ElementType> ResolveDuplicateTypes(Document targetDocument, SystemTypeSyncPlanItem syncItem)
	{
		...
		Dictionary<string, ElementType> currentTargetMap = BuildSystemTypeMap(targetDocument);
		foreach (string duplicateName in syncItem.RelatedDuplicateNames.OrderBy<string, string>([SpecialName] (string x) => x, StringComparer.OrdinalIgnoreCase))
		{
	...
	{
		return (dependencyPlan?.Items ?? new List<RoutingDependencyPreflightItem>()).Where([SpecialName] (RoutingDependencyPreflightItem x) => string.Equals(Normalize(x.SystemFamilyKind), Normalize(systemFamilyKind), StringComparison.Ordinal) && string.Equals(Normalize(x.SystemTypeName), Normalize(typeName), StringComparison.Ordinal)).OrderBy<RoutingDependencyPreflightItem, string>([SpecialName] (RoutingDependencyPreflightItem x) => Normalize(x.DependencyRole), StringComparer.Ordinal).ThenBy<RoutingDependencyPreflightItem, string>([SpecialName] (RoutingDependencyPreflightItem x) => Normalize(x.SourceFamilyName), StringComparer.Ordinal)
			.ThenBy<RoutingDependencyPreflightItem, string>([SpecialName] (RoutingDependencyPreflightItem x) => Normalize(x.SourceTypeName), StringComparer.Ordinal);
	}
