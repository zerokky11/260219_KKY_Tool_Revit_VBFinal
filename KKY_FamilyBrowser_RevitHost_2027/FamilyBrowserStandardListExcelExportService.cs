...
public sealed class FamilyBrowserStandardListExcelExportService
{
	...
	private static List<List<string>> BuildRowsFromSnapshot(string defaultDiscipline, StandardLibrarySnapshot snapshot, FamilyBrowserStandardListCatalog catalog, string disciplineKey, string disciplineLabel)
	{
		...
		foreach (StandardLoadableFamilySnapshotItem item in (snapshot.LoadableFamilies ?? new List<StandardLoadableFamilySnapshotItem>()).Where([SpecialName] (StandardLoadableFamilySnapshotItem x) => x != null).OrderBy<StandardLoadableFamilySnapshotItem, string>([SpecialName] (StandardLoadableFamilySnapshotItem x) => Normalize(x.CategoryName), StringComparer.Ordinal).ThenBy<StandardLoadableFamilySnapshotItem, string>([SpecialName] (StandardLoadableFamilySnapshotItem x) => Normalize(x.FamilyName), StringComparer.Ordinal))
		{
		...
		foreach (StandardSystemTypeSnapshotItem item2 in (snapshot.SystemTypes ?? new List<StandardSystemTypeSnapshotItem>()).Where([SpecialName] (StandardSystemTypeSnapshotItem x) => x != null).OrderBy<StandardSystemTypeSnapshotItem, string>([SpecialName] (StandardSystemTypeSnapshotItem x) => Normalize(x.CategoryName), StringComparer.Ordinal).ThenBy<StandardSystemTypeSnapshotItem, string>([SpecialName] (StandardSystemTypeSnapshotItem x) => Normalize(x.TypeClassName), StringComparer.Ordinal)
			.ThenBy<StandardSystemTypeSnapshotItem, string>([SpecialName] (StandardSystemTypeSnapshotItem x) => Normalize(x.TypeName), StringComparer.Ordinal))
		{
	...
	private static List<List<string>> BuildFingerprintAuditRows(string defaultDiscipline, FamilyBrowserStandardListCatalog catalog, StandardLibrarySnapshot snapshot, string disciplineKey, string disciplineLabel, bool korean, FamilyBrowserStandardFingerprintAuditExportResult result)
	{
		...
		checked
		{
			foreach (FamilyBrowserStandardListEntry entry in entries.Where([SpecialName] (FamilyBrowserStandardListEntry x) => x != null && StandardListEntryAppliesToDiscipline(x, CS_0024_003C_003E8__locals10._0024VB_0024Local_disciplineKey, CS_0024_003C_003E8__locals10._0024VB_0024Local_disciplineLabel, CS_0024_003C_003E8__locals10._0024VB_0024Local_hasDisciplineSpecificRows)).OrderBy<FamilyBrowserStandardListEntry, string>([SpecialName] (FamilyBrowserStandardListEntry x) => x.Category ?? string.Empty, StringComparer.OrdinalIgnoreCase).ThenBy<FamilyBrowserStandardListEntry, string>([SpecialName] (FamilyBrowserStandardListEntry x) => x.Family ?? string.Empty, StringComparer.OrdinalIgnoreCase)
				.ThenBy<FamilyBrowserStandardListEntry, string>([SpecialName] (FamilyBrowserStandardListEntry x) => x.TypeName ?? string.Empty, StringComparer.OrdinalIgnoreCase))
			{
