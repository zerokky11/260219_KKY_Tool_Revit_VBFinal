...
public sealed class ProjectSnapshotFingerprintService
{
	...
	public static string NormalizeRoutingPreferenceSignature(string signature)
	{
		...
		return string.Join("\n", lines.OrderBy<string, string>([SpecialName] (string x) => x, StringComparer.Ordinal));
	}
	...
	private static string BuildTypeNameSignature(IEnumerable<string> typeNames)
	{
		...
		return Normalize(string.Join("|", typeNames.OrderBy<string, string>([SpecialName] (string x) => Normalize(x), StringComparer.Ordinal)));
	}
