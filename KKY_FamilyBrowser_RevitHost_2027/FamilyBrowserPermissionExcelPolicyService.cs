...
public sealed class FamilyBrowserPermissionExcelPolicyService
{
	...
	{
		string text = (value ?? string.Empty).Trim().TrimEnd(new char[2] { '\\', '/' });
	...
	private static bool IsPathUnderFolder(string candidatePath, string applyFolder)
	{
		...
		else
		{
			try
			{
				string fullPath = Path.GetFullPath(pathText).TrimEnd(new char[2] { '\\', '/' });
				string fullFolder = Path.GetFullPath(folderText).TrimEnd(new char[2] { '\\', '/' });
