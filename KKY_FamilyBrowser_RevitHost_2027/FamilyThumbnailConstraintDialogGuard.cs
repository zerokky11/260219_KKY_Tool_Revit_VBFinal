...
public sealed class FamilyThumbnailConstraintDialogGuard : IDisposable
{
	...
	private sealed class NativeFamilyScanDialog
	{
		...
		public NativeFamilyScanDialog(nint handle, string title, string body, NativeFamilyScanDialogAction action, string reason)
		{
			Handle = handle;
	...
	private sealed class NativeWindowButton
	{
		...
		public NativeWindowButton(nint handle, string text, bool enabled, int controlId)
		{
			Handle = handle;
	...
	private nint _lastNativeDialogHandle;
	...
	private static NativeFamilyScanDialog FindNativeFamilyScanDialog()
	{
		...
		EnumWindows([SpecialName] (nint hWnd, nint lParam) =>
		{
	...
	private static bool TryClickNativeDialogAction(NativeFamilyScanDialog dialog, ref string clickedButtonText)
	{
		...
		clickedButtonText = (string.IsNullOrWhiteSpace(button.Text) ? ((object)button.Handle/*cast due to .constrained prefix*/).ToString() : button.Text);
	...
	private static List<string> GetDescendantTexts(nint parentHandle)
	{
		List<string> list = new List<string>();
		EnumChildWindows(parentHandle, [SpecialName] (nint childHandle, nint lParam) =>
		{
	...
	private static List<NativeWindowButton> GetChildButtons(nint parentHandle)
	{
	...
	[DllImport("user32.dll")]
	private static extern uint GetWindowThreadProcessId(nint hWnd, ref uint processId);
