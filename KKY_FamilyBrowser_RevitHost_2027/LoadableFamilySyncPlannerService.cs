...
public sealed class LoadableFamilySyncPlannerService
{
	...
	public static LoadableFamilySyncPlan BuildPlan(ProjectStandardComparisonReport comparisonReport, string comparisonPath, bool allowFingerprintOverwrite = false, bool allowMissingFamilyLoad = false)
	{
		...
		checked
		{
			foreach (LoadableFamilyComparisonItem item in comparisonReport.LoadableFamilies.OrderBy<LoadableFamilyComparisonItem, string>([SpecialName] (LoadableFamilyComparisonItem x) => Normalize(x.CategoryName) + "|" + Normalize(x.FamilyName), StringComparer.Ordinal))
			{
