...
import { clear, div, toast, setBusy, showExcelSavedDialog, debounce, chooseExcelMode, showCompletionSummaryDialog } from '../core/dom.js';
...
export function renderParamProp(root) {
    ...
    function handleDone({ ok, status, message, report, details, rows }) {
        ...
        lastProgressPct = 0;
        ProgressDialog.hide();
        requestAnimationFrame(() => showParamPropCompletionDialog(good));
    }
    ...
    function onExport() {
        ...
        chooseExcelMode((mode) => {
            ...
            ProgressDialog.show('Excel Export', 'Preparing...');
            ProgressDialog.update(0, 'Preparing...', '');
            post('sharedparam:export-excel', { excelMode: mode || 'fast' });
        });
    }
    ...
    function showParamPropCompletionDialog(good) {
        const counts = summarizeDetailTypes();
        const summaryLine = String(state.lastReport || '')
            .split(/\r?\n/)
            .map((line) => line.trim())
            .find(Boolean);
        const notes = [];
        if (summaryLine && summaryLine !== DEFAULT_GUIDE) notes.push(summaryLine);

        showCompletionSummaryDialog({
            title: good ? '공유 파라미터 연동 완료' : '공유 파라미터 연동 결과',
            message: '작업이 끝났습니다. 결과 표에서 상세를 확인하고 필요하면 바로 엑셀로 내보낼 수 있습니다.',
            summaryItems: [
                { label: '상세 결과 건수', value: String(state.lastDetails.length || 0) },
                { label: 'ScanFail', value: String(counts.ScanFail || 0) },
                { label: 'Skip', value: String(counts.Skip || 0) },
                { label: 'Error', value: String(counts.Error || 0) },
                { label: 'ChildError', value: String(counts.ChildError || 0) }
            ],
            notes,
    ...
    function summarizeDetailTypes() {
        return (state.lastDetails || []).reduce((acc, row) => {
            const key = String(row?.type || row?.kind || '').trim();
            if (!key) return acc;
            acc[key] = (acc[key] || 0) + 1;
            return acc;
        }, {});
