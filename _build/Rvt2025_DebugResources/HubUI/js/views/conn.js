...
export function renderConn(root) {
  ...
  paramResultWrap.classList.add('conn-param-result-wrap');
  ...
  paramInput.classList.add('conn-param-input');
  ...
  const excludeCheckbox = excludeEndDummy;
  ...
  const commit = () => {
    ...
    saveOpts({
      ...
      includePointXY: Boolean(includePointXY.checked),
      includeLinearMetrics: Boolean(includeLinearMetrics.checked),
  ...
  includePointXY.addEventListener('change', commit);
  includeLinearMetrics.addEventListener('change', commit);
