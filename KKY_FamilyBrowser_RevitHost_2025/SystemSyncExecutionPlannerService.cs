...
public sealed class SystemSyncExecutionPlannerService
{
	...
	public static SystemSyncExecutionPlan BuildPlan(SystemTypeSyncPlan syncPlan, RoutingDependencyPreflightPlan dependencyPlan)
	{
		...
		Dictionary<string, List<RoutingDependencyPreflightItem>> dependencyMap = (dependencyPlan?.Items ?? new List<RoutingDependencyPreflightItem>()).GroupBy<RoutingDependencyPreflightItem, string>([SpecialName] (RoutingDependencyPreflightItem x) => BuildKey(x.SystemFamilyKind, x.SystemTypeName), StringComparer.Ordinal).ToDictionary<IGrouping<string, RoutingDependencyPreflightItem>, string, List<RoutingDependencyPreflightItem>>([SpecialName] (IGrouping<string, RoutingDependencyPreflightItem> x) => x.Key, [SpecialName] (IGrouping<string, RoutingDependencyPreflightItem> x) => x.OrderBy<RoutingDependencyPreflightItem, string>([SpecialName] (RoutingDependencyPreflightItem y) => Normalize(y.DependencyRole), StringComparer.Ordinal).ThenBy<RoutingDependencyPreflightItem, string>([SpecialName] (RoutingDependencyPreflightItem y) => Normalize(y.SourceFamilyName), StringComparer.Ordinal).ThenBy<RoutingDependencyPreflightItem, string>([SpecialName] (RoutingDependencyPreflightItem y) => Normalize(y.SourceTypeName), StringComparer.Ordinal)
			.ToList(), StringComparer.Ordinal);
		foreach (SystemTypeSyncPlanItem syncItem in syncPlan.Items.OrderBy<SystemTypeSyncPlanItem, string>([SpecialName] (SystemTypeSyncPlanItem x) => Normalize(x.SystemFamilyKind), StringComparer.Ordinal).ThenBy<SystemTypeSyncPlanItem, string>([SpecialName] (SystemTypeSyncPlanItem x) => Normalize(x.SourceTypeName), StringComparer.Ordinal))
		{
