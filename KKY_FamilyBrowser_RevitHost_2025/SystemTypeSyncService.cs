...
public sealed class SystemTypeSyncService
{
	...
	public static SystemTypeSyncPlan BuildPlan(SystemTypeCatalogSnapshot sourceCatalog, SystemTypeCatalogSnapshot targetCatalog)
	{
		...
		Dictionary<string, List<SystemTypeSemanticSnapshot>> targetNameGroups = targetTypes.GroupBy<SystemTypeSemanticSnapshot, string>([SpecialName] (SystemTypeSemanticSnapshot x) => BuildNameKey(x.SystemFamilyKind, x.TypeName), StringComparer.Ordinal).ToDictionary<IGrouping<string, SystemTypeSemanticSnapshot>, string, List<SystemTypeSemanticSnapshot>>([SpecialName] (IGrouping<string, SystemTypeSemanticSnapshot> x) => x.Key, [SpecialName] (IGrouping<string, SystemTypeSemanticSnapshot> x) => x.ToList(), StringComparer.Ordinal);
		using (IEnumerator<SystemTypeSemanticSnapshot> enumerator = sourceCatalog.Types.OrderBy<SystemTypeSemanticSnapshot, string>([SpecialName] (SystemTypeSemanticSnapshot x) => Normalize(x.SystemFamilyKind), StringComparer.Ordinal).ThenBy<SystemTypeSemanticSnapshot, string>([SpecialName] (SystemTypeSemanticSnapshot x) => Normalize(x.TypeName), StringComparer.Ordinal).GetEnumerator())
		{
			...
			while (enumerator.MoveNext())
			{
				...
				categoryMismatchTargets = categoryMismatchTargets.Where(closure_0024__3_002D._Lambda_0024__6).OrderBy<SystemTypeSemanticSnapshot, string>([SpecialName] (SystemTypeSemanticSnapshot x) => Normalize(x.CategoryName), StringComparer.Ordinal).ThenBy<SystemTypeSemanticSnapshot, string>([SpecialName] (SystemTypeSemanticSnapshot x) => Normalize(x.TypeName), StringComparer.Ordinal)
					.ToList();
				List<SystemTypeSemanticSnapshot> relatedDuplicates = targetTypes.Where(closure_0024__3_002D._Lambda_0024__9).Where(closure_0024__3_002D._Lambda_0024__10).Where(closure_0024__3_002D._Lambda_0024__11)
					...
					.ToList();
				item.RelatedDuplicateNames.AddRange(relatedDuplicates.Select([SpecialName] (SystemTypeSemanticSnapshot x) => x.TypeName).OrderBy<string, string>([SpecialName] (string x) => x, StringComparer.OrdinalIgnoreCase));
				if (exactTarget == null)
				{
					if (categoryMismatchTargets.Count > 0)
					{
						item.Action = "ManualReview";
						item.DestinationTypeName = string.Join(", ", categoryMismatchTargets.Select([SpecialName] (SystemTypeSemanticSnapshot x) => x.TypeName).Distinct<string>(StringComparer.OrdinalIgnoreCase).OrderBy<string, string>([SpecialName] (string x) => x, StringComparer.OrdinalIgnoreCase));
						item.DiffSummary.Add("Category mismatch: standard category " + (closure_0024__3_002D._0024VB_0024Local_sourceType.CategoryName ?? string.Empty) + " / project category " + string.Join(", ", categoryMismatchTargets.Select([SpecialName] (SystemTypeSemanticSnapshot x) => x.CategoryName).Distinct<string>(StringComparer.OrdinalIgnoreCase).OrderBy<string, string>([SpecialName] (string x) => x, StringComparer.OrdinalIgnoreCase)));
	...
	private static List<string> BuildDiffSummary(SystemTypeSemanticSnapshot sourceType, SystemTypeSemanticSnapshot destinationType)
	{
		...
		foreach (string key in sourceKeys.Except<string>(destinationKeys, StringComparer.Ordinal).OrderBy<string, string>([SpecialName] (string x) => x, StringComparer.Ordinal))
		{
			diffs.Add("Type parameter missing in destination: " + key);
		}
		foreach (string key2 in destinationKeys.Except<string>(sourceKeys, StringComparer.Ordinal).OrderBy<string, string>([SpecialName] (string x) => x, StringComparer.Ordinal))
		{
			diffs.Add("Extra destination type parameter: " + key2);
		}
		using (IEnumerator<string> enumerator3 = sourceKeys.Intersect<string>(destinationKeys, StringComparer.Ordinal).OrderBy<string, string>([SpecialName] (string x) => x, StringComparer.Ordinal).GetEnumerator())
		{
