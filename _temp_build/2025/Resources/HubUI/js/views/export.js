...
function buildPreviewSummary(summary) {
    ...
    const previewFileCount = countPreviewFiles(state.rowsRaw);

    const selectedFileCount = Number.isFinite(selectedCountRaw) ? selectedCountRaw : selectedFilePaths().length;
    const successFileCount = Number.isFinite(successCountRaw) ? successCountRaw : (previewFileCount || state.rowsRaw.length);
...
function countPreviewFiles(rows) {
    const files = new Set();
    (Array.isArray(rows) ? rows : []).forEach((row) => {
        const key = String(row?.File || row?.file || '').trim().toLowerCase();
