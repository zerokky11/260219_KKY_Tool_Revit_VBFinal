...
export function renderDup(root){
  ...
  function route(ev, payload){
    try{
      ...
      if (ev === 'dup:progress'){
        if (payload && String(asStr(payload.kind, '')).toLowerCase() === 'run'){
  ...
  function normalizeRow(r){
    ...
    var comment = asStr(co(r.comment, co(r.Comment, co(r.reason, co(r.Reason, '')))), '');
    var groupKey = asStr(co(r.groupKey, r.GroupKey), '');
    var connRaw = co(r.connectedIds, co(r.ConnectedIds, []));
    var connectedIds = [];
    if (Array.isArray(connRaw)){
      connectedIds = connRaw.map(String);
    } else if (typeof connRaw === 'string'){
      connectedIds = connRaw.split(/[,\s;|]+/).map(function(x){ return String(x || '').trim(); }).filter(Boolean);
    }
    return { id: id, category: category, family: family, type: type, comment: comment, groupKey: groupKey, connectedIds: connectedIds };
  }
  ...
  function handleRunProgress(p){
    if (!p){ ProgressDialog.hide(); return; }
    var pct = clamp(asNum(p.percent, 0), 0, 100);
    var current = asNum(p.current, 0);
    var total = asNum(p.total, 0);
...
function ensureStyles(){
  ...
  var css = [
    ...
    '.grp-body .dup-subhead,.grp-body .dup-row{display:grid;grid-template-columns:44px 92px 1fr 1.05fr 1.05fr 1.35fr 76px 190px;gap:10px;align-items:center;padding:8px 12px;}',
    ...
    '@media (max-width:1100px){.grp-body .dup-subhead,.grp-body .dup-row{grid-template-columns:44px 92px 0.95fr 0.95fr 0.95fr 1.2fr 66px 176px;gap:8px;}}',
    ...
    '@media (max-width:860px){.grp-body .dup-subhead,.grp-body .dup-row{grid-template-columns:44px 92px 0.9fr 0.95fr 0.9fr 1.1fr 0.7fr 160px;}}'
  ].join('\n');
