...
function buildWorkspaceCard(state) {
  ...
  viewPanel.innerHTML = `
    ...
    <section class="deliverycleaner-subsection">
      <div class="deliverycleaner-subsection__head">
        <h4>VV 서브카테고리 자동 끄기</h4>
        <p>한 규칙에 여러 주 카테고리를 쉼표로 입력하고, XML로 저장/불러오기 하면서 정리용 3D 뷰의 서브카테고리 숨김 기준을 맞출 수 있습니다.</p>
      </div>
      <div class="deliverycleaner-section-callout deliverycleaner-section-callout--compact">
        <div class="deliverycleaner-note">기존 End Cut 자동 해제 규칙은 제거되고, 아래 커스텀 규칙만 적용됩니다.</div>
        ...
        <div class="deliverycleaner-inline-controls">
          <div class="deliverycleaner-inline-select">
            <span>Imported Categories 표시</span>
          ...
          <div class="deliverycleaner-inline-select">
            <span>Imports in Families 표시</span>
  ...
  state.ui.visibilityCombinationMode = viewPanel.querySelector('[data-visibility-combination-mode]');
  state.ui.visibilityImportBtn = viewPanel.querySelector('[data-visibility-import]');
  state.ui.visibilitySaveBtn = viewPanel.querySelector('[data-visibility-save]');
  state.ui.visibilityImportedToggle = viewPanel.querySelector('[data-visibility-imported-toggle]');
  state.ui.visibilityImportsFamilyToggle = viewPanel.querySelector('[data-visibility-imports-family-toggle]');
  state.ui.visibilityRuleBody = viewPanel.querySelector('[data-visibility-rule-body]');
  state.ui.visibilitySummary = viewPanel.querySelector('[data-visibility-summary]');
...
function renderVisibilityRuleRows(state) {
  ...
  state.visibilityRules.rules.forEach((row, index) => {
    const tr = document.createElement('tr');
    tr.append(
      tdWithTextarea((row.parentCategoryNames || []).join(', '), '예: Duct Fitting, Pipe Fitting', (value) => {
        state.visibilityRules.rules[index].parentCategoryNames = splitVisibilityCategoryNames(value);
        renderVisibilityRuleSummary(state);
      }),
...
function renderVisibilityRuleSummary(state) {
  ...
  state.ui.visibilitySummary.textContent = '주 카테고리들을 쉼표로 입력하고, 서브카테고리 텍스트 조건을 지정하면 커스텀 VV 숨김 규칙으로 반영됩니다.';
}
